...
def greeting():
    ...
    if hour >=6 and hour < 12:
        Speak("Günaydın efendim", True)
    elif hour >= 12 and hour <18:
        Speak("Iyi günler efendim", True)
    else:
        Speak("Iyi akşamlar efendim", True)
...
while True:
    ...
    #     Speak("Yapay zekaya geçiliyor", True)
    #     while True:
    #         query = takecommand()
    #         if query:
    #             if query == "çıkış yap":
    ...
    if query =="uyuyabilirsin"  or query =="uyu" or query == "görüşmek üzere" or query == "görüşürüz":
        Speak("Tamam efendim, bana ihtiyacınız olursa seslenebilirsiniz", True)
